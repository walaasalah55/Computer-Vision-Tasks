...
class Snake:
    ...
    def __init__( self, image = None, closed = True ):
        ...
        self.gray = self.from_RGB_to_GS(self.image)
        self.binary = self.thresholding(self.gray, 120)
        self.gradientX, self.gradientY = self.Sobel_Edgd_Detection(self.gray)
        print(len(self.gradientX), len(self.gradientY), self.gray.shape[0])
...
######################################################################
    def dist( a, b ):
        """ return: The euclidian distance between the two points a , b """
    ...
    def get_length(self):
        ...
        for i in range( 0, n_points ):
           snake_len = np.sum( Snake.dist( self.points[i], self.points[ (i+1)%n_points ] ) )
        
        return snake_len

    
    def get_area_shoelace(self):
        xl=[]
        yl=[]
        a1,a2=0,0
        n_points = len(self.points)
        ...
        for i in range( 0, n_points ):
            xl.append(self.points[i][0])
            yl.append(self.points[i][1])
...
# Sobel
###########################################################
    def Calculate_Size_After_Applying_Kernel(self, img_size: int, kernel_size: int) -> int:
        num_pixels = 0
    ...
    def Convolution(self, img: np.array, kernel: np.array) -> np.array:
        Image_size = self.Calculate_Size_After_Applying_Kernel(
    ...
    def Sobel_Edgd_Detection(self, img: np.array):
        ...
        Vertical_Sobel = self.Convolution(img=np.array(img), kernel=sobel_Kernel_X)
        Horizontal_Sobel = self.Convolution(img=np.array(img), kernel=sobel_Kernel_Y)
        print(len(Vertical_Sobel), len(Horizontal_Sobel))
        ...
        return Vertical_Sobel, Horizontal_Sobel
######################################################################
# Eenergy functions
######################################################################
    ...
    def f_edge( self, p ):
        ...
        # If the point is out of the bounds of the image, return a high value
        # (since it is a minimization problem)
        if p[0] < 0 or p[0] >= self.width or p[0] >= len(self.gradientX) or p[1] < 0 or p[1] >= self.height or p[1] >= len(self.gradientY):
            return np.finfo(np.float64).max
...
######################################################################
    ...
    def step( self ):
        ...
        self.snake_area = self.get_area_shoelace()
        # print("The perimeter =", self.snake_length)
        # print("The area =", self.snake_area)
